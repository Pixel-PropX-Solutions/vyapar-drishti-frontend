...
import { updateOrder, updateOrderDetails, viewOrderDetails } from '@/services/order';
...
const UpdateOrderProduct = () => {
  ...
  useEffect(() => {
    dispatch(setProductDataNull())              // set current
    if (productData) {
      setCurrentProduct(productData);
    }
    if (orderDetailsData && orderDetailsData.orders && orderDetailsData.orders.length > 0) {
      setSelectedProducts(orderDetailsData.orders.map((product) => ({
        product_name: product?.ProductDetails?.product_name,
        product_id: product?.ProductDetails?._id,
        quantity: product?.product_details?.quantity,
        unit_price: product?.product_details?.unit_price
      })));
    }
  }, [productData, orderDetailsData, dispatch]);
  ...
  const handleSubmitOrder = async () => {
    ...
    setSubmitting(true);
    try {
      await dispatch(updateOrderDetails({ data: selectedProducts, order_id: orderId ?? "" })).unwrap().then(() => {
        const data = {
