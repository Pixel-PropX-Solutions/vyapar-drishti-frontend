...
import { UpdateInventoryGroup } from "@/utils/types";
import { createInventoryGroup, updateInventoryGroup, viewAllInventoryGroups } from "@/services/inventoryGroup";

interface CreateInventoryGroupModalProps {
    open: boolean;
    onClose: () => void;
    onCreated: (inventoryGroup: { name: string; _id: string }) => void;
    onUpdated?: () => Promise<void>;
    inventoryGroup?: UpdateInventoryGroup | null;
}
...
const CreateInventoryGroupModal: React.FC<CreateInventoryGroupModalProps> = ({
    open,
    onClose,
    onCreated,
    onUpdated,
    ...
    const inventoryGroupOptions = inventoryGroupLists?.map(group => ({
        label: group.inventory_group_name,
        value: group.inventory_group_name,
    })) || [];
    ...
    const validateForm = (): boolean => {
        ...
        if (!data.inventory_group_name.trim()) {
            errors.inventory_group_name = 'Group name is required';
        } else if (data.inventory_group_name.length < 2) {
            errors.inventory_group_name = 'Group name must be at least 2 characters';
        }
    ...
    const handleBoxClick = () => {
        inventoryGroupfileInputRef.current?.click();
    };
    ...
    const removeImage = useCallback((e: React.MouseEvent) => {
        ...
        if (inventoryGroupfileInputRef.current) {
            inventoryGroupfileInputRef.current.value = '';
        }
    }, []);
    ...
    useEffect(() => {
        ...
        if (open && inventoryGroup) {
            setData({
                inventory_group_name: inventoryGroup.inventory_group_name || '',
                description: inventoryGroup.description || '',
                image: inventoryGroup.image || '',
                parent: inventoryGroup.parent || '',
            });
            setImagePreview(
                typeof inventoryGroup?.image === "string" ? inventoryGroup.image : null
            );
    ...
    const handleSubmit = async () => {
        ...
        const sanitizedData: any = {
            inventory_group_name: data.inventory_group_name.trim(),
        };
        ...
        if (inventoryGroup && inventoryGroup._id) {
            // Edit mode
            await toast.promise(
                dispatch(updateInventoryGroup({
                    id: inventoryGroup._id,
                    data: formData
                }))
                    .unwrap()
                    .then(() => {
                    ...
                    <Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="h6" fontWeight={700}>
                                {inventoryGroup ? 'Edit Group' : 'Create Group'}
                            </Typography>
                        </Box>
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                            {inventoryGroup ? 'Update the details of your group' : 'Create a new group for your products'}
                        </Typography>
                    </Box>
                </Box>
                    ...
                    >
                        ...
                        <Box sx={{ mb: 2 }}>
                            ...
                            <input
                                ...
                                style={{ display: 'none' }}
                                ref={inventoryGroupfileInputRef}
                                onChange={e => {
                        ...
                        <Stack spacing={2}>
                            ...
                            <FormControl fullWidth>
                                ...
                                <TextField
                                    ...
                                    value={data.inventory_group_name}
                                    onChange={(e) => handleInputChange('inventory_group_name', e.target.value)}
                                    error={!!formErrors.inventory_group_name}
                                    helperText={formErrors.inventory_group_name}
                                    sx={{
                ...
                >
                    <Stack direction={isMobile ? "column" : "row"} spacing={2} sx={{ width: isMobile ? '100%' : 'auto' }}>
                        ...
                        <Button
                            ...
                            disabled={isLoading || !data.inventory_group_name.trim()}
                            fullWidth={isMobile}
                            sx={{
                                textTransform: 'none',
                                px: 4,
                        ...
                        >
                            {isLoading
                                ? (inventoryGroup ? 'Updating Group...' : 'Creating Group...')
                                : (inventoryGroup ? 'Update Group' : 'Create Group')
                            }
                        </Button>
                    </Stack>
...
export default CreateInventoryGroupModal;